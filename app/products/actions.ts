...
async function context(action: PermissionKey | "MANAGE_CATALOG" | "MANAGE_INVENTORY" | "MANAGE_PHOTOS") {
  const session = await requireRouteAccess("/products");
  const mapped: PermissionKey = action === "MANAGE_CATALOG" ? "CATALOG_EDIT" : action === "MANAGE_INVENTORY" ? "INVENTORY_RECEIVE" : action === "MANAGE_PHOTOS" ? "CATALOG_PHOTO_MANAGE" : action;
  await requirePermission(session, mapped);
...
export async function createProductAction(form: FormData) {
  try { const { tenant } = await context("CATALOG_CREATE"); const product = await createProduct(tenant, productInput(form)); revalidatePath("/products"); redirect(`/products/${product.id}?ok=${encodeURIComponent("Товар создан.")}`); } catch (error) { if ((error as { digest?: string }).digest?.startsWith("NEXT_REDIRECT")) throw error; go("/products/new", "error", errorMessage(error)); }
}
...
export async function archiveProductAction(form: FormData) { const id = text(form, "productId"); try { const { tenant } = await context("CATALOG_ARCHIVE"); await archiveProduct(tenant, id); revalidatePath("/products"); go("/products", "ok", "Товар архивирован."); } catch (error) { go(`/products/${id}`, "error", errorMessage(error)); } }
...
export async function adjustStockAction(form: FormData) { const productId=text(form,"productId"); try { const {tenant,session}=await context("INVENTORY_ADJUST"); await adjustBulkStock(tenant,{variantId:text(form,"variantId"),branchId:text(form,"branchId"),locationId:nullable(form,"locationId"),delta:integer(form,"delta"),reason:text(form,"reason"),userId:session.userId}); revalidatePath(`/products/${productId}`); go(`/products/${productId}`,"ok","Остаток скорректирован."); } catch(error){ go(`/products/${productId}`,"error",errorMessage(error)); } }
