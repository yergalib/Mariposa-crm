...
const operationalExecutionCode=(value:string)=>{const map:Record<string,string>={а:"A",б:"B",в:"V",г:"G",д:"D",е:"E",ё:"E",ж:"ZH",з:"Z",и:"I",й:"Y",к:"K",л:"L",м:"M",н:"N",о:"O",п:"P",р:"R",с:"S",т:"T",у:"U",ф:"F",х:"H",ц:"TS",ч:"CH",ш:"SH",щ:"SCH",ъ:"",ы:"Y",ь:"",э:"E",ю:"YU",я:"YA"};return [...value.normalize("NFKC").toLowerCase()].map(c=>map[c]??(/[a-z0-9]/.test(c)?c.toUpperCase():"-")).join("").replace(/-+/g,"-").replace(/^-|-$/g,"").slice(0,70)||"EXECUTION"};
...
export async function buildFullCatalogPlan(workbookPath:string,namespace:NamespaceSnapshot){
  ...
  const planSha256=createHash("sha256").update(JSON.stringify(core)).digest("hex").toUpperCase();const executionApplyCodes:Record<string,string>={};const usedByProduct=new Map<string,Set<string>>();for(const execution of executions){const used=usedByProduct.get(execution.productId)??new Set<string>();let code=operationalExecutionCode(execution.name);if(used.has(code))code=`${code.slice(0,62)}-${execution.id.slice(0,7).toUpperCase()}`;if(used.has(code))throw new Error(`Execution operational code collision: ${execution.name}`);used.add(code);usedByProduct.set(execution.productId,used);executionApplyCodes[execution.id]=code}return {...core,planSha256,executionApplyCodes};
}

export type FullCatalogPlan=Awaited<ReturnType<typeof buildFullCatalogPlan>>;
