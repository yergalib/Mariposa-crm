...
export async function getDashboard(tenant: TenantContext, input: DashboardInput, actor: Actor) {
  ...
  return db.$transaction(async (tx) => {
    ...
    const bulkResolutionRows=allocationIds.length?await tx.bulkPhysicalResolution.findMany({where:{organizationId:tenant.organizationId,capacityAllocationId:{in:allocationIds},kind:{in:["LOSS_RESOLUTION","RETURN"]}},select:{orderId:true,kind:true,totalQuantity:true,lines:{where:{outcome:{in:["DAMAGED","LOST"]}},select:{id:true}}}}):[];
    const lossesByOrder=new Map<string,number>();for(const row of bulkResolutionRows)if(row.kind==="LOSS_RESOLUTION")lossesByOrder.set(row.orderId,(lossesByOrder.get(row.orderId)??0)+row.totalQuantity);
    ...
    const settlementOrderIds = orders.filter((x) => x.capacityAllocations.some((a) => a.issuedQuantity > 0)).map((x) => x.id), damagedAllocationIds = orders.flatMap((x) => x.capacityAllocations.filter((a) => a.returnedAt && a.returnInspectionResult === "DAMAGED" && a.productInstanceId).map((a) => a.id)),damagedBulkLineIds=bulkResolutionRows.flatMap(row=>row.lines.map(line=>line.id)),damagedSourceIds=[...damagedAllocationIds,...damagedBulkLineIds];
    ...
    const damageWaivers = settlementOrderIds.length && damagedSourceIds.length ? await tx.auditLog.findMany({ where: { organizationId: tenant.organizationId, action: "ORDER_DAMAGE_WAIVED", entityType:{in:["CapacityAllocation","BulkPhysicalResolutionLine"]}, entityId: { in: damagedSourceIds } }, select: { entityId: true } }) : [];
    ...
    for (const order of orders) {
      ...
      const issued = order.capacityAllocations.reduce((s, a) => s + a.issuedQuantity, 0), returned = order.capacityAllocations.reduce((s, a) => s + a.returnedQuantity, 0),lost=lossesByOrder.get(order.id)??0, outstanding = Math.max(issued - returned-lost, 0), commercial = order.items.reduce((s, x) => s + x.quantity, 0), remaining = Math.max(commercial - issued, 0), terminal = TERMINAL.includes(order.status as never);
      ...
      if (issued > 0) {
        const rows = settlementFinance.filter((x) => x.orderId === order.id), damaged = order.capacityAllocations.filter((a) => a.returnedAt && a.returnInspectionResult === "DAMAGED" && a.productInstanceId).map(a=>a.id),bulkDamaged=bulkResolutionRows.filter(r=>r.orderId===order.id).flatMap(r=>r.lines.map(l=>l.id)),damageSources=[...damaged,...bulkDamaged];
        const unresolved = damageSources.filter((sourceId) => !waived.has(sourceId) && !rows.some((x) => x.kind === "DAMAGE_CHARGE" && x.sourceType === "RETURN_DAMAGE_ASSESSMENT" && x.sourceId === sourceId && !x.reversal)).length;
        ...
        const settlement = evaluateReturnSettlement({ orderStatus: order.status, issuedQuantity: issued, returnedQuantity: returned,resolvedUnrecoverableQuantity:lost, unresolvedDamageCount: unresolved, outstandingMinor: obligation, heldDepositMinor: deposit, damageChargeMinor: damage, damageWithheldMinor: withheld, activeDamageChargeCount });
