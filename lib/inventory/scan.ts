...
export type InventoryScanResult =
  | { kind: "BULK_VARIANT"; code: string; variantId: string; productId: string; productName: string; size: string; sku: string }
  | { kind: "SERIALIZED_INSTANCE"; code: string; instanceId: string; variantId: string; productId: string; productName: string; size: string; barcode: string; inventoryNumber: string; branchId: string };
...
export async function resolveInventoryScan(
  ...
  return db.$transaction(async (tx) => {
    ...
    const variants = await tx.productVariant.findMany({
        ...
        select: { id: true, productId: true, sku: true, product: { select: { name: true } }, size: { select: { code: true, name: true } } }
        ,take: 2
      });
    const instances = await tx.productInstance.findMany({
        ...
        select: { id: true, productVariantId: true, inventoryNumber: true, barcode: true, currentBranchId: true, productVariant: { select: { productId: true, product: { select: { name: true } }, size: { select: { code: true, name: true } } } } }
        ,take: 2
      });
    ...
    if (classified?.kind === "BULK_VARIANT") return { kind: "BULK_VARIANT", code, variantId: variant!.id, productId: variant!.productId, productName: variant!.product.name, size: variant!.size.name || variant!.size.code, sku: variant!.sku };
    ...
    return { kind: "SERIALIZED_INSTANCE", code, instanceId: instance.id, variantId: instance.productVariantId, productId: instance.productVariant.productId, productName: instance.productVariant.product.name, size: instance.productVariant.size.name || instance.productVariant.size.code, barcode: instance.barcode, inventoryNumber: instance.inventoryNumber, branchId: instance.currentBranchId };
