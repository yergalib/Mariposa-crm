...
BEGIN
  IF TG_TABLE_NAME='bulk_physical_resolutions' THEN
    IF TG_OP='DELETE' THEN resolution_id := OLD."id"; ELSE resolution_id := NEW."id"; END IF;
  ELSE
    IF TG_OP='DELETE' THEN resolution_id := OLD."resolution_id"; ELSE resolution_id := NEW."resolution_id"; END IF;
  END IF;
