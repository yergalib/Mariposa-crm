...
async function main(){
 ...
 const importBatches=await db.catalogImportBatch.findMany({select:{id:true,status:true,metadata:true}});const interrupted=importBatches.filter(batch=>batch.id==="6a1eec99-b691-5bba-ac6a-fa96814f0d87"&&batch.status==="APPLYING"&&batch.metadata&&typeof batch.metadata==="object"&&!Array.isArray(batch.metadata)&&(batch.metadata as Record<string,unknown>).stage==="INTERRUPTED");
 pass("all public tables retain RLS",posture.tables===posture.rls);pass("anon privileges zero",posture.anon===0);pass("authenticated privileges zero",posture.authenticated===0);pass("granting policies zero",posture.policies===0);pass("migration count 40",posture.migrations===40);pass("only expected interrupted import batch exists",posture.batches===1&&importBatches.length===1&&interrupted.length===1);pass("interrupted import created no references",posture.references===0);pass("four provenance triggers enabled",triggers.length===4&&triggers.every(t=>t.enabled==="O"));pass("provenance constraints present",constraints.length===5);
