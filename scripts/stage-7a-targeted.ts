...
async function cleanup() {
  for (const organizationId of organizations) await db.$transaction(async (tx) => {
    ...
    await tx.inventoryMovement.deleteMany({ where: { organizationId } }); await tx.instanceStatusHistory.deleteMany({ where: { organizationId } }); await tx.instanceConditionHistory.deleteMany({ where: { organizationId } }); await tx.productInstance.deleteMany({ where: { organizationId } });
